 ...
 * the file system itself is just a gzipped bflz-zipped mkfs archive
 * prefixed with "gzfilesystem N\n" and suffixed with
 * a kilobyte of zeros.
 *
...
char zero[512];
int
blockread(int in, char *first, int nfirst, int total)
{
	int p[2], out, n, rv;
	char blk[512];
	...
	while(total > 0 && (n=read(in, blk, sizeof blk)) > 0){
		if(n > total)
			n = total;
		total -= n;
...
main(int argc, char **argv)
{
	...
	/* search for archive on block boundary */
if(chatty) fprint(2, "searching for gzfilesystem\n");
	for(i=0;; i++){
		if((n = readn(fd, blk, sizeof blk)) != sizeof blk)
			sysfatal("read %d gets %d: %r\n", i, n);
		if(strncmp(blk, "gzfilesystem ", 13) == 0) {
			q = strchr(blk, '\n');
			if(q != nil) {
				start = q+1;
				startlen = blk+sizeof(blk)-start;
				len = atoi(blk+13);
				break;
			}
		}
...
if(chatty) fprint(2, "found at %d\n", i);
	...
	fd = unbflz(ungzip(blockread(fd, start, startlen, len)));
